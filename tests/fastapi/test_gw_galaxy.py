...
class TestGWGalaxyEndpoints:
    ...
    def test_post_event_galaxies_with_doi(self):
        ...
        if not timesent_stamp:
            pytest.skip(f"Alert for {graceid} does not have timesent field in test data")
    ...
    def test_get_glade_galaxies_by_name(self):
        ...
        if len(data) > 0:
            for galaxy in data:
                ...
                for name_field in ["_2mass_name", "gwgc_name", "hyperleda_name", "sdssdr12_name"]:
                    if name_field in galaxy and galaxy[name_field] and "NGC" in galaxy[name_field]:
                        has_name_match = True
                        break
    ...
    def test_authentication_required(self):
        ...
        endpoints = [
            "/event_galaxies?graceid=S190425z",
            "/glade"
        ]
